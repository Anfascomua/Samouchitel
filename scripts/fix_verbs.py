...
with tarfile.open(fileobj=io.BytesIO(raw),mode="r:gz") as tf:
    ...
    for ef in sorted(n for n in names if n.endswith(".en")):
        ...
        for en,ru in zip(ens,rus):
            if not ru or not 3<=len(TOK.findall(en))<=20: continue
            low=en.lower(); toks=set(TOK.findall(low))
            for k,(pres,past) in forms.items():
                b=cand[k]
                ...
                elif toks & past: t="past"
                elif toks & pres: t="present"
                if t and len(b[t])<5: b[t].append((en.strip(),ru.strip()))
fixed=0
